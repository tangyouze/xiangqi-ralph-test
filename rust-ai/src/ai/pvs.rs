...
impl PVSAI {
    ...
    fn order_moves_inplace(
        ...
        let countermove =
            last_move.and_then(|m| self.countermove[m.from_pos.to_index()][m.to_pos.to_index()]);
    ...
    fn pvs(
        ...
        if !is_pv
            && !in_check
            && depth >= NULL_MOVE_DEPTH_LIMIT
            && self.has_non_pawn_pieces(board, color)
        {
        ...
        let prev_best = if (depth as usize) < 64 {
            self.best_move_at_depth[depth as usize]
        } else {
            None
        };
        self.order_moves_inplace(
            board,
            &mut legal_moves,
            color,
            ply as usize,
