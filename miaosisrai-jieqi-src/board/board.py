...
class Board:
    ...
    def is_legal_board(self, board=None):
        ...
        for element in common.RED_POSITIONS:
            ...
            j = element[1]
            if (
                board[i][j] & common.MASK_CHESS_ISCOVERED != 0
                and board[i][j] & common.MASK_COLOR == 0
            ):
        ...
        for element in common.BLACK_POSITIONS:
            ...
            j = element[1]
            if (
                board[i][j] & common.MASK_COLOR != 0
                and board[i][j] & common.MASK_CHESS_ISCOVERED != 0
            ):
    ...
    def random_board(self):
        ...
        # 安置帅将
        RED_SHUAI_POSSIBLE_POSITIONS = (
            common.RED_JIUGONG - fixed_covered_chesses
        )  # 红帅不能和红暗士重叠
        ...
        BLACK_JIANG_POSSIBLE_POSITIONS = (
            common.BLACK_JIUGONG - fixed_covered_chesses
        )  # 黑将不能和黑暗士重叠
        BLACK_JIANG_POS = random.sample(BLACK_JIANG_POSSIBLE_POSITIONS, 1)[0]
        new_board[BLACK_JIANG_POS[0]][BLACK_JIANG_POS[1]] = chessdict[BLACK_JIANG_POS] = (
            common.BLACK_JIANG
        )
        other_possible_positions = (
            common.ALL_POSITIONS - {RED_SHUAI_POS} - {BLACK_JIANG_POS} - fixed_covered_chesses
        )
        uncovered_counter = deepcopy(common.UNCOVERED_COUNTER)
        # 将帅已安置，不能重复考虑
        ...
        red_bing_positions = common.random_select(
            other_possible_positions - common.RED_BING_FORBIDDEN,
            return_type=set,
            num=uncovered_counter.get(15, 0),
        )[1]
        for pos in red_bing_positions:
            chessdict[pos] = new_board[pos[0]][pos[1]] = 15
        black_zu_positions = common.random_select(
            other_possible_positions - common.BLACK_ZU_FORBIDDEN - red_bing_positions,
            return_type=set,
            num=uncovered_counter.get(7, 0),
        )[1]
        for pos in black_zu_positions:
            chessdict[pos] = new_board[pos[0]][pos[1]] = 7
        uncovered_counter[15] = uncovered_counter[7] = 0
        other_possible_positions = (
            other_possible_positions - red_bing_positions - black_zu_positions
        )
        ...
        # 选取存活的明子
        number_of_alived_uncovered_chesses, alive_uncovered_chesses = common.random_select(
            all_uncovered_chesses
        )
        positions_of_alived_uncovered_chesses = random.sample(
            other_possible_positions, number_of_alived_uncovered_chesses
        )
    ...
    def random_board_uncertainty(self):
        ...
        # 计算剩余位置, 2021/05/22
        possible_uncertainties_positions = other_possible_positions - set(
            positions_of_alived_uncovered_chesses
        )
        ...
        if red > 0:
            ...
            _, set_red_uncertainty_positions = common.random_select(
                possible_uncertainties_positions, return_type=set, num=num_of_red_uncertainties
            )
        ...
        if black > 0:
            ...
            _, set_black_uncertainty_positions = common.random_select(
                possible_uncertainties_positions, return_type=set, num=num_of_black_uncertainties
            )
       ...
       """
        ...
        if chess_type not in (1, 6):  # 车炮比较特殊
            if chess_type == 2:  # 马
                ...
                if (
                    board[(src[0] + dst[0]) // 2][(src[1] + dst[1]) // 2] != 0
                ):  # 象眼位置: ((src[0] + dst[0])//2, (src[1] + dst[1])//2)
                    return False
                ...
                if ((turn and dst == jiang) or (not turn and dst == shuai)) and self.drink(
                    board=board, shuaijiang=shuaijiang
                ):  # 判断饮酒
                    return True
                if (turn and dst not in common.RED_JIUGONG) or (
                    (not turn) and dst not in common.BLACK_JIUGONG
                ):
                    return False
            ...
            for x2 in range(self.H):
                for y1 in range(self.W):
                    for y2 in range(self.W):
                        islegal, isjiangjun = self.check_legal_and_jiangjun(
                            (x1, y1), (x2, y2), board=None, turn=None, shuaijiang=None
                        )
                        drink = self.drink()
                        if counter % 10 == 0:
    ...
    def stupid_generate_all_legal_moves(self, board=None, turn=None, shuaijiang=None):
        ...
        for x1 in range(self.H):
            for x2 in range(self.H):
                for y1 in range(self.W):
                    for y2 in range(self.W):
                        islegal = self.check_legal(
                            (x1, y1), (x2, y2), board=board, turn=turn, shuaijiang=shuaijiang
                        )
                        if islegal:
    ...
    def reverse_move(self, t):
        return (
            chr(Board.ALL - ord(t[0]))
            + str(9 - int(t[1]))
            + chr(Board.ALL - ord(t[2]))
            + str(9 - int(t[3]))
        )
    ...
    def generate(self, turn, check=False, file=False, verbose=False, uncertainty=False):
        ...
        legal_moves = list(
            self.get_legal_moves_speedup(
                board=board, turn=turn, shuaijiang=shuaijiang, chessdict=chessdict
            )
        )
        if check:
            legal_moves_2 = self.stupid_generate_all_legal_moves(
                board=board, turn=turn, shuaijiang=shuaijiang
            )
        ...
        if check:
            ...
            print(
                "translate_uncovered_board: mapping --translate_mapping-> newmapping --uncover_board_icybee-> translate_uncovered_board"
            )
            self.print_board_icybee(translate_uncovered_board)
            print(
                "translate_uncovered_board: board --uncover_board-> uncovered_board --> translate_uncovered_board2"
            )
    ...
    def scan(self, board):
        ...
        for i in range(self.H):
            for j in range(self.W):
                if board[i][j] == 0:
                    continue
                if (board[i][j] & common.MASK_CHESS_UNCERTAIN > 0) or (
                    board[i][j] & common.MASK_CHESS_ISCOVERED > 0
                ):
    ...
    def evaluate(self, board, dict_red, dict_black, verbose=False):
        ...
        averager, averageb = {}, {}
        lenkeyr, lenkeyb = (
            sum(dict_red[key] for key in dict_red),
            sum(dict_black[key] for key in dict_black),
        )
    ...
    def get_legal_moves_speedup(self, board=None, turn=None, shuaijiang=None, chessdict=None):
        ...
        for pos in chessdict:
            ...
            if (turn and (chess & common.MASK_COLOR == 0)) or (
                (not turn) and (chess & common.MASK_COLOR == common.MASK_COLOR)
            ):
                continue
            chess_type = chess & common.MASK_CHESS
            if chess_type == 1:  # 车
            ...
            if chess_type == 3:  # 相
                if pos[0] < self.H - 2:
                    if (
                        pos[1] < self.W - 2
                        and board[pos[0] + 1][pos[1] + 1] == 0
                        and _helper((pos[0] + 2, pos[1] + 2)) != turn
                    ):
            ...
            if chess_type == 4:  # 仕
                if (
                    board[pos[0]][pos[1]] & common.MASK_CHESS_ISCOVERED
                    == common.MASK_CHESS_ISCOVERED
                ):  # 暗士
                    ...
                    if (
                        pos[0] > 0
                        and pos[1] < self.W - 1
                        and _helper((pos[0] - 1, pos[1] + 1)) != turn
                    ):
                        yield (pos[0], pos[1], pos[0] - 1, pos[1] + 1)
                    if (
                        pos[0] < self.H - 1
