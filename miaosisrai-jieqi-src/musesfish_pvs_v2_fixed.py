...
class Position(namedtuple("Position", "board score turn")):
    ...
    def value(self, move):
        ...
        # Actual move
        if p == "H" and (
            (i == 164 and j == 52 and self.board[51] in "dr")
            or (i == 170 and j == 58 and self.board[59] in "dr")
        ):  # TODO: 使用更智能的方式处理博子
        ...
        if p in "RNBAKCP":
            ...
            score = (
                average[self.turn][True][j] - average[self.turn][False]
            )  # 相应位置不确定明子的平均价值 - 暗子
            if p == "D":
                key = "R" if not self.turn else "r"  # 对方车
                score -= 20 + 40 * (
                    di[not self.turn][key] + self.che_opponent
                )  # 暗车溜出，扣分! 扣的分数和对方剩余车的个数有关
            if p == "I":
        ...
        if q.isupper():
            ...
            if q in "RNBAKCP":
                ...
                if q == "D":
                    ...
                    score += (
                        20 + 40 * (di[self.turn][key] + self.che)
                    )  # 吃对方暗车，加分! 加的分数和己方剩余车的个数相关，如果本方没有车了，那吃个暗车不算太大的收益
