...
sumall = {
    0: {
        True: sum(di[0][True][key] for key in di[0][True]),
...
class Position(namedtuple("Position", "board score turn version")):
    ...
    def set(self):
        ...
        if (self.kongtoupao > 0 and self.kongtoupao_opponent <= 0) or (
            self.kongtoupao > self.kongtoupao_opponent > 0
        ):
        ...
        elif (self.kongtoupao <= 0 and self.kongtoupao_opponent > 0) or (
            self.kongtoupao_opponent > self.kongtoupao > 0
        ):
            if (
                self.che_opponent >= self.che and self.che_opponent > 0
            ) or self.kongtoupao_opponent >= 3:
    ...
    def mymove_check(self, move, discount_red=True, discount_black=False):
        ...
        if self.board[i] in "RNBAKCP":
            ...
            if self.turn:
                ...
                di[self.version][False][dst2] -= 1
        board = put(board, i, ".")
        sumall[self.version][True] = sum(
            di[self.version][True][key] for key in di[self.version][True]
        )
        sumall[self.version][False] = sum(
            di[self.version][False][key] for key in di[self.version][False]
        )
    ...
    def calc(self):
        shi_possibility = (
            0
            if sumall[self.version][not self.turn] == 0
            else di[self.version][not self.turn]["a" if self.turn else "A"]
    ...
    def value(self, move):
        ...
        possible_che = (
            ...
            * di[self.version][self.turn]["R" if self.turn else "r"]
            / sumall[self.version][self.turn]
        )
        possible_che_opponent = (
            0
            if sumall[self.version][not self.turn] == 0
            else self.covered_opponent
        ...
        if p in "RNBAKCP":
            ...
            if p == "C":
                ...
                if (
                    self.board[51] not in "dr"
                    and self.board[54] != "a"
                    and self.board[71] != "a"
                    and (self.board[71] == "p" or self.board[87] != "n")
                ):
                ...
                if (
                    (i >> 4) == 3
                    and (j >> 4) != 3
                    and (self.endline <= 1 or self.score_rough < -150)
                ):
                    if self.endline <= 1:
                        score += 30
            ...
            score = (
                average[self.version][self.turn][True][j]
                - average[self.version][self.turn][False]
                + 20
            )  # 相应位置不确定明子的平均价值 - 暗子
            ...
            if p == "D":
                ...
                if (
                    i == 196 and j == 165 and self.board[149] == "I"
                ):  # 对方车从3,7线杀出，翻动暗马保住暗兵
                ...
                if (
                    ...
                    if (
                        cheonleidao > che_opponent_onleidao
                        and possible_che >= possible_che_opponent
                    ):
                        score += 40 * self.calc()
                ...
                if (
                    ...
                    di[self.version][self.turn]["P" if self.turn else "p"]
                    * self.covered
                    / sumall[self.version][self.turn]
                    <= 2
                ):
                ...
                if (i == 164 and j == 52 and self.board[52] == "e" and self.board[51] in "dr") or (
                    i == 170 and j == 58 and self.board[58] == "e" and self.board[59] in "dr"
                ):  # TODO: 使用更智能的方式处理博子
                    if (i == 164 and self.board[148] == "p") or (
                        i == 170 and self.board[154] == "p"
                    ):
                        pass
                    if self.che < self.che_opponent or self.che == 0 or self.score_rough < 150:
                        ...
                        if self.score_rough > 200:
                            pass
                        elif (i == 164 and (self.board[131] != "N" or self.board[115] != "p")) and (
                            i == 170 and (self.board[139] != "N" or self.board[123] != "p")
                        ):
                            score -= 30
...
class Searcher:
    ...
    def quiescence(self, pos, moves, oppo):
        ...
        if (
            argmax
            and pos.board[argmax[1]] == "c"
            and argmax[1] & 15 == 7
            and pos.kongtou_score_opponent > 0
            and pos.kongtoupao_opponent > 0
        ):
    ...
    def alphabeta(self, pos, alpha, beta, depth, root=True, nullmove=False, nullmove_now=False):
        ...
        if nullmove_now and depth > 3 and not root and any(c in pos.board for c in "RNCI"):
            if all(oppo.board[m[1]] != "k" for m in oppo.gen_moves()):
                ...
                if val >= beta and self.alphabeta(
                    pos, alpha, beta, depth - 3, root=False, nullmove=nullmove, nullmove_now=False
                ):
                    return val
    ...
    def alphabeta_double_recursive(
        self,
        pos,
        alpha,
        beta,
        ...
        if nullmove_now and depth > 3 and not root and any(c in pos.board for c in "RNCI"):
            if all(oppo.board[m[1]] != "k" for m in oppo.gen_moves()):
                val = -self.alphabeta_double_recursive(
                    pos.rotate(),
                    -beta,
                    1 - beta,
                    depth - 3,
                ...
                if val >= beta and self.alphabeta_double_recursive(
                    pos,
                    alpha,
                    beta,
                    depth - 3,
        ...
        if uncertainty_dict[key] == "U":
            for di_key in di[version][turn]:
                if di[version][turn][di_key] > 0 and di_key in "RPCrpc":
                    ...
                    self._inner_recur(
                        ...
                        score + score_diff,
                        turn,
                        me=me * (di[version][turn][di_key] + 1),
                        op=op,
                        pruning=pruning,
                    )
                    di[version][turn][di_key] += 1
            ...
            for di_key in di[version][notturn]:
                if di[version][notturn][di_key] > 0 and di_key in "RPCrpc":
                    ...
                    rkey = 254 - key
                    score_diff = (
                        pst[di_key.upper()][rkey] - average[version - 1][notturn][True][rkey]
                    )
                    ###################################################################################
                    ...
                    self._inner_recur(
                        ...
                        turn,
                        me=me,
                        op=op * (di[version][notturn][di_key] + 1),
                        pruning=pruning,
                    )
                    di[version][notturn][di_key] += 1
    ...
    def calc_average(self, version=0):
        numr, numb = (
            sum(di[version][True][key] for key in di[version][True]),
            sum(di[version][False][key] for key in di[version][False]),
        )
        ...
        self.average = {
            True: {False: averagecoveredr, True: averager},
            False: {False: averagecoveredb, True: averageb},
        }
    ...
    for move in moves:
        ...
        if check_bozi:
            ...
            if p == "H" and (
                ...
                or (i == 170 and j == 58 and pos.board[59] in "dr")
            ):  # TODO: 使用更智能的方式处理博子
                if (i == 164 and pos.board[131] == "N" and pos.board[115] == "p") or (
                    j == 170 and pos.board[139] == "N" and pos.board[123] == "p"
                ):  # 兵顶马
                    continue
