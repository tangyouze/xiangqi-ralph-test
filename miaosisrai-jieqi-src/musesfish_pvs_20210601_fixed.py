...
from board import common_20210601_fixed as common
    ...
    def value(self, move):
        ...
        if p in "RNBAKCP":
            ...
            if self.board[i - 32] == "i":
                score -= (
                    average[self.version][not self.turn][True][270 - i]
                    - average[self.version][not self.turn][False]
                )
            ...
            if p == "D":
                ...
                if (i == 164 and j == 52 and self.board[51] == "d") or (
                    i == 170 and j == 58 and self.board[59] == "d"
                ):  # TODO: 使用更智能的方式处理博子
                    if self.che < self.che_opponent or self.che == 0 or self.score_rough < 150:
                        score -= 100
                    elif self.che == self.che_opponent:
                        if (
                            (i == 164 and self.board[148] == "p")
                            or (i == 170 and self.board[154] == "p")
                            or self.score_rough > 200
                        ):
                            pass
...
class Searcher:
    ...
    def alphabeta(self, pos, alpha, beta, depth, root=True, nullmove=False):
        ...
        if nullmove and depth > 0 and not root and any(c in pos.board for c in "RNC"):
            val = -self.alphabeta(
                pos.nullmove(), -beta, 1 - beta, depth - 3, root=False, nullmove=nullmove
            )
            if val >= beta and self.alphabeta(
                pos, alpha, beta, depth - 3, root=False, nullmove=nullmove
            ):
                return val
        ...
        for move in [killer] + moves:
            ...
            if (move is not None) and (depth > 0):
                if best == -MATE_UPPER:
                    val = -self.alphabeta(
                        pos.move(move), -beta, -alpha, depth - 1, root=False, nullmove=nullmove
                    )
                else:
                    val = -self.alphabeta(
                        pos.move(move), -alpha - 1, -alpha, depth - 1, root=False, nullmove=nullmove
                    )
